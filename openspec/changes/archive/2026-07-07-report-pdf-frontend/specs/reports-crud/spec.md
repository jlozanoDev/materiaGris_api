...
The system SHALL update `docs/funcional/modulos/informes.md` to describe: report creation with patient+template selection, draft saving, signing workflow, closing workflow, and **PDF download (stored file for archived reports, client-side generation for signed reports)**.
(Previously: described server-side PDF generation via Dompdf for all PDF operations.)

#### Scenario: Business rules for PDF operations
